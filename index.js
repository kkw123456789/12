...
client.on("message", message => {
  if (message.content === prefix + "명령어") {
    const embed = new Discord.MessageEmbed()
      ...
      .addField(`\`${prefix}목록 / 큐\``, "To View Queue Song", true)
      .addField(`\`${prefix}현재\``, "To View Now Playing", true)
      .addField(`\`${prefix}반복 / 반복\``, "To Change Repeat Mode (off, song, queue)", true)
  ...
  if (message.content === prefix + "전체취소" || message.content.startsWith(prefix + "나가")) {
    ...
    if (!queue) return message.channel.send(new Discord.MessageEmbed()
      .setTitle("❌ 에러r")
      .setColor("RED")
      .setDescription("지금은 대기열에 아무것도 없습니다!"))
    client.distube.stop(message);
    const embed = new Discord.MessageEmbed()
      .setTitle("⏸️  정지")
      .setColor("RED")
      .setDescription(`노래가 성공적으로 중지되었습니다.!`)
    message.channel.send(embed)
  }
  ...
  if (message.content === prefix + "반복재생" || message.content.startsWith(prefix + "반복")) {
    ...
    const embed = new Discord.MessageEmbed()
      .setTitle("🔄  반복")
      .setColor("RED")
      .setDescription(`노래가 반복 되었습니다 \`${mode}\``)
    message.channel.send(embed)
  }
  ...
  if (message.content === prefix + "볼륨설정" || message.content.startsWith(prefix + "볼륨")) {
    ...
    if (!args) return message.channel.send(new Discord.MessageEmbed()
      .setTitle("❌  에러")
      .setColor("RED")
      .setDescription(`볼륨을 설정하려면 숫자를 입력하세요`)
    );
    if (!message.member.voice.channel)
      return message.channel.send(new Discord.MessageEmbed()
        .seTitle("❌  에러")
    ...
    if (volume > 100) return message.channel.send(new Discord.MessageEmbed()
      .setTitle("❌  에러")
      .setColor("RED")
      .setDescription(`최대 볼륨은 \`100\``)
    )
    client.distube.setVolume(message, volume)
    const embed = new Discord.MessageEmbed()
      .setTitle("🔊  소리")
      .setColor("RED")
      .setDescription(`소리는 \`${volume}\``)
    message.channel.send(embed)
  }
  if (message.content === prefix + "재정") {
    ...
    if (queue.pause) {
      ...
      return message.channel.send(new Discord.MessageEmbed()
        .setTitle("▶️  재개됨")
        .setColor("RED")
        .setDescription(`노래가 다시 재생 되었습니다!`)
      )
